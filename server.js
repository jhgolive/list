...
async function fetchEventsForDate(dateIso, datePretty) {
  ...
  if (!links.length) {
    ...
    const warningLine = isDateSum18(dateIso) ? `💢 ${datePretty} 사고 조심  0건` : `✨ ${datePretty}  0건`;
  ...
  const warningLine = isDateSum18(dateIso) ? `💢 ${datePretty} 사고 조심  ${results.length}건` : `✨ ${datePretty}  ${results.length}건`;
